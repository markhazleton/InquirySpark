...
Add a RESTful, HATEOAS-driven Conversation API to `InquirySpark.Admin` that guides an application user through a structured survey experience in discrete steps. The API exposes two primary endpoints — `/start` and `/next` — and uses hypermedia links so a thin client never needs to know workflow structure. No new database tables are created; the implementation relies entirely on the existing `ApplicationUser`, `Survey`, `QuestionGroup`, `QuestionGroupMember`, `Question`, `QuestionAnswer`, `SurveyResponse`, and `SurveyResponseAnswer` entities.
...
**Independent Test**: Can be fully tested with a REST client (e.g., Swagger UI or curl) by calling `/start`, following `next_url` through each question step, and verifying a `SurveyResponse` with `SurveyResponseAnswer` rows exists in the database after the final step.
...
A user who started a survey but did not finish can resume from their last unanswered question, or deliberately restart the survey from the beginning with all prior answers cleared, by passing the existing `response_id` to `/start`.
...
**Independent Test**: Start a conversation, answer two questions, then call `POST /api/v1/conversation/start` with the same `response_id` and no `action`; verify the third question (first unanswered) is returned and the two earlier `SurveyResponseAnswer` rows are intact.

**Acceptance Scenarios**:

1. **Given** an in-progress `SurveyResponse` with two questions answered, **When** `POST /api/v1/conversation/start` is called with `response_id` and no `action`, **Then** the first unanswered question is returned and no existing answers are modified.
2. **Given** a `SurveyResponse` with answers, **When** `POST /api/v1/conversation/start` is called with `response_id` and `action: "restart"`, **Then** all `SurveyResponseAnswer` rows for that response are deleted, `StatusId` is reset to in-progress, and question 1 is returned.
3. **Given** a `response_id` that belongs to a different user, **When** `POST /api/v1/conversation/start` is called, **Then** `400 Bad Request` is returned and no data is modified.
...
- **FR-002**: `POST /api/conversation/start` with valid credentials and no `survey_id` MUST return a list of available surveys (`SurveyNm`, `SurveyShortNm`, `SurveyDs`, `SurveyId`) plus `action_type: "survey_selection"`.
- **FR-003**: `POST /api/conversation/start` behavior depends on whether `response_id` is supplied:
  - **No `response_id`**: Always creates a new `SurveyResponse` linked to the authenticated `ApplicationUser` and scoped to the specified `Application`. `survey_id` is required in this case.
  - **`response_id` + default or `action: "resume"`**: Locates the existing `SurveyResponse`, verifies it belongs to the authenticated user, and returns the next unanswered question (the first `Question` in ordered sequence that has no corresponding `SurveyResponseAnswer`).
  - **`response_id` + `action: "restart"`**: Deletes all `SurveyResponseAnswer` rows for that `SurveyResponseId`, resets `SurveyResponse.StatusId` to in-progress, and returns question 1.
  - If `application_id` is absent or unknown, the API MUST return `400 Bad Request`. If a supplied `response_id` does not belong to the authenticated user, the API MUST return `400 Bad Request` (not `404`, to avoid user enumeration).
- **FR-004**: Every response from `/start` and `/next` MUST follow the same envelope schema: `response_id`, `action` (containing `action_type` and `question` or `completion`), `next_url`, `prev_url` (when applicable), and `conversation_ended`.
- **FR-005**: `POST /api/conversation/next/{response_id}/{question_id}` with an answer body MUST persist the answer as a `SurveyResponseAnswer` and return the next unanswered question, or set `conversation_ended: true` when no more questions remain.
- **FR-006**: `POST /api/conversation/next/{response_id}/{question_id}` without an answer body MUST return the current question for that step without modifying state (read mode).
...
- **FR-010**: The API MUST NOT create, modify, or reference any database table that does not already exist in the current schema.
...
- **FR-012**: The `next_url` in every response MUST be a fully-qualified relative URL the client can POST to without constructing its own path. The `{question_id}` segment in `next_url` MUST be the `Question.QuestionId` primary key value as returned by the server.
...
- **FR-015**: The Conversation API endpoints MUST be accessible without an ASP.NET Core Identity session or cookie. They MUST be decorated as anonymous endpoints. The `account_name` and `password` fields in the request body are the sole authentication mechanism; no additional authorization layer is required.
...
- **SurveyResponse**: The conversation session. One record per user–survey run. `StatusId` tracks `in_progress` vs `complete`. Linked to `ApplicationUser` (`AssignedUserId`) and `Survey` (`SurveyId`).
...
```json
{
  ...
  "next_url": "/api/v1/conversation/next/42/7",
  "prev_url": "/api/v1/conversation/next/42/6",
...
| `POST` | `/api/v1/conversation/next/{response_id}/{question_id}` | Submit answer and advance; or read current step |
...
| `response_id` | No | Existing `SurveyResponse.SurveyResponseId` to resume or restart. If omitted, a new response is always created. |
| `action` | No | `"resume"` (default) or `"restart"`. Only meaningful when `response_id` is supplied. `"restart"` deletes all `SurveyResponseAnswer` rows for the response and returns question 1. |

**Responses:**
- `200` — action_type `survey_selection` (no survey_id), `question` (survey started/resumed/restarted)
- `400` — `application_id` missing/unknown, survey inactive, survey has no questions, or `response_id` does not belong to the authenticated user
- `401` — credentials invalid
- `404` — supplied `response_id` not found

### POST `/api/v1/conversation/next/{response_id}/{question_id}`
...
- `404` — response_id or question_id not found
...
- **SC-002**: Every answered question produces exactly one `SurveyResponseAnswer` row; duplicate submissions for the same question overwrite the existing row rather than creating duplicates.
...
- Q: What does `{question_id}` represent in the URL path `/next/{response_id}/{question_id}`? → A: The database `Question.QuestionId` primary key. Clients never construct or compute this value; they read it from `next_url` returned by the server.
...
- Q: Can the same user take the same survey more than once, and can an existing response be updated? → A: Yes to both. If `response_id` is omitted on `/start`, a new `SurveyResponse` is always created. If `response_id` is supplied, the default behavior is **resume** (return the next unanswered question). Supplying `action: "restart"` alongside `response_id` clears all existing `SurveyResponseAnswer` rows for that response and restarts from question 1.
- Q: Must the Conversation API be protected by ASP.NET Core Identity session/cookie auth in addition to body credentials? → A: No — the endpoints are anonymous (no `[Authorize]` middleware). The `account_name` + `password` fields in the request body are the sole authentication mechanism.
...
7. When `response_id` is supplied on `/start` with `action: "restart"`, the deletion of `SurveyResponseAnswer` rows is the only destructive operation — the `SurveyResponse` record itself is preserved and reused.
...
5. No new tables, migrations, or schema changes are introduced.
